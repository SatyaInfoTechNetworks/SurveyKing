...
// GET /api/telegram/streak
async function getStreakStatus(req, res) {
  try {
    const tgUserId = String(req.query.telegramUserId || '');
    if (!tgUserId) return res.status(400).json({ error: 'telegramUserId required' });
...
// POST /api/telegram/streak/claim
async function claimStreakAd(req, res) {
  try {
    ...
    if (lastClaimed) {
      const lastClaimedStr = new Date(lastClaimed).toISOString().split('T')[0];
      if (lastClaimedStr === todayStr) {
        return res.status(400).json({ error: 'Daily streak already claimed today! Check back tomorrow.' });
      }
    }
    ...
    if (lastClaimed) {
      const lastClaimedStr = new Date(lastClaimed).toISOString().split('T')[0];
      if (lastClaimedStr === yesterdayStr) {
    ...
    if (streakRows.length > 0) {
      ...
      await db.execute('INSERT INTO user_daily_streaks (user_id, current_streak, last_claimed_date) VALUES (?, ?, ?)', [user.id, newStreak, todayStr]);
    }
    ...
    await db.execute(
      ...
      [user.id, rewardCoins, `STREAK_CLAIM_D${newStreak}_${Date.now()}`, `Daily Streak Day ${newStreak} Reward (+${rewardCoins} Coins)`]
    );

    return res.json({
      success: true,
      message: `🎉 Day ${newStreak} Streak Claimed! +${rewardCoins} Coins added to your balance.`,
      newStreak,
      rewardCoins,
      newBalance
    });
    ...
    return res.status(500).json({ error: 'Failed to claim daily streak' });
  }
}
