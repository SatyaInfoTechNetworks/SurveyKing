...
// -------------------------------------------------------------------
// 3. ADSGRAM REWARD WEBHOOK ENDPOINT
// GET /api/webhooks/adsgram?userid=[userid]
// -------------------------------------------------------------------
async function handleAdsgramWebhook(req, res) {
  const startTime = Date.now();
  const clientIp = req.clientIp || '127.0.0.1';
  const tgUserId = req.query.userid || req.query.user_id || req.query.telegramUserId;

  console.log(`📺 [ADSGRAM WEBHOOK] Incoming Reward GET Request for Telegram User ID: ${tgUserId}`);
  ...
  try {
    const userRows = await db.query('SELECT * FROM users WHERE telegram_user_id = ?', [String(tgUserId)]);
    if (userRows.length === 0) {
      console.warn(`⚠️ Adsgram Webhook: User with Telegram ID ${tgUserId} not found.`);
